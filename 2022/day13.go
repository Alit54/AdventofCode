...
func compare(packet1, packet2 Packet) int {
	...
	// Other cases
	len := Alit.Max(len(packet1.num)+len(packet1.list), len(packet2.num)+len(packet2.list))
	for i := 0; i < len; i++ {
		...
		if packet1.num[i] != 0 {
			return compare(convert(packet1.num[i]), packet2.list[i])
		}
		if packet2.num[i] != 0 {
			return compare(packet1.list[i], convert(packet2.num[i]))
		}
